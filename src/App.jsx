...
export default function App() {
  ...
  useEffect(function () {
    ...
    if (local) {
      ...
      setTarget(JSON.parse(localStorage.getItem('heartfull_TARGET')) ?? 100);
    } else {
  ...
  function handleMainClick(data = 1) {
    ...
    if (settingsActive) setSettingsActive(false);
      ...
      const latestLoop =
        qalma.loop + Number((qalma.count + data) % target === 0 && data > 0);
  ...
  return (
    <>
      ...
      <Main handleMainClick={handleMainClick}>
        <Qalma qalma={qalma} />
        <Counter count={(qalma?.count ?? 0) % (target + 1)} target={target} />
